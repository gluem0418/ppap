...
onMounted(() => {
  if (sceneContainer.value) {
    ...
    // const camera = new THREE.OrthographicCamera(-(rect.width / 2), rect.width / 2, rect.height / 2, -(rect.height / 2), 1, 1000);
    ...
    window.addEventListener('click', (event) => {
      ...
      if (intersects.length > 0) {
        ...
        const scalePointY = point.y * scaleFactor
        ...
        for (let i = 0; i < starCount / 2; i++) {
          ...
          starVertices[i * 3] = scalePointX + starShapeVertices[index * 3];
          starVertices[i * 3 + 1] = scalePointY + starShapeVertices[index * 3 + 1];
